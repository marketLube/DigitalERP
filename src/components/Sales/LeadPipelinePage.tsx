...
const LeadCard: React.FC<LeadCardProps> = ({ lead, onEdit, onDelete, onStatusChange, compact = false, cardMode = 'colorful' }) => {
  ...
  const getPriorityGradient = (priority: string) => {
    switch (priority) {
      case 'Hot': return 'from-red-50 via-red-25 to-orange-50';
      case 'Warm': return 'from-yellow-50 via-amber-25 to-orange-50';
      case 'Cold': return 'from-blue-50 via-sky-25 to-indigo-50';
      default: return 'from-gray-50 via-gray-25 to-slate-50';
    }
  };
  ...
  const getPriorityBorder = (priority: string) => {
    switch (priority) {
      ...
      case 'Warm': return 'border-l-yellow-400';
      case 'Cold': return 'border-l-blue-400';
      default: return 'border-l-gray-400';
    }
  };

  // Card background and border styling based on mode
  const cardStyling = cardMode === 'minimal' 
    ? `bg-white border-l-4 ${getPriorityBorder(lead.priority)} border-t border-r border-b border-gray-200`
    : `bg-gradient-to-br ${getPriorityGradient(lead.priority)} border border-gray-200`;

  const statuses = ['New Leads', 'Qualified', 'Proposal', 'Negotiation', 'Closed Won'];

  return (
    <div className={`w-full max-w-full overflow-hidden rounded-lg p-3 hover:shadow-md transition-all duration-200 group h-[145px] flex flex-col ${cardStyling}`}>
      {/* Header: Title + Dropdown */}
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1 min-w-0">
          <h4 className="font-poppins font-semibold text-gray-900 text-sm mb-1 line-clamp-1 truncate">
            {lead.title}
          </h4>
          {/* Combined Client + Contact on single line with hover */}
          <div className="text-xs text-gray-600 font-poppins truncate" title={`${lead.company} • ${lead.contactPerson}`}>
            {lead.company} • {lead.contactPerson}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-base font-poppins font-semibold text-green-600">
          ${lead.value.toLocaleString()}
        </span>
        {cardMode === 'colorful' && (
          <span className={`text-xs px-2 py-0.5 rounded-full border font-poppins font-medium ${getPriorityColor(lead.priority)}`}>
            {lead.priority}
          </span>
        )}
      </div>

      {/* Horizontal Probability Bar with Icon + Label + Bar + Percentage */}
      <div className="mb-2">
        <div className="flex items-center gap-2">
          <Target size={11} className="text-blue-600 flex-shrink-0" />
          <span className="text-xs text-gray-600 flex-shrink-0">Progress</span>
          ...
          <span className="text-xs font-poppins font-medium text-gray-900 flex-shrink-0">{lead.probability}%</span>
        </div>
      </div>

      {/* Communication Icons + Assignee */}
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1 flex-shrink-0">
          <button className="p-0.5 hover:bg-gray-100 rounded transition-colors" title={`Email: ${lead.email}`}>
            <Mail size={11} className="text-gray-500" />
          </button>
          <button className="p-0.5 hover:bg-gray-100 rounded transition-colors" title={`Phone: ${lead.phone}`}>
            <Phone size={11} className="text-gray-500" />
          </button>
          <button className="p-0.5 hover:bg-gray-100 rounded transition-colors" title="Send Message">
            <MessageCircle size={11} className="text-gray-500" />
          </button>
        </div>
        
        {/* Assignee Avatars - Limit to 2 visible + overflow */}
        <div className="flex items-center max-w-[60px] overflow-hidden">
          <div className="flex items-center -space-x-1">
            <div className="w-4 h-4 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0" title={lead.assignee}>
              <span className="text-white text-[8px] font-medium">{lead.assignee.charAt(0)}</span>
            </div>
            <div className="w-4 h-4 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0" title="Co-assignee">
              <span className="text-white text-[8px] font-medium">+</span>
            </div>
          </div>
          <span className="text-[8px] text-gray-500 ml-1 truncate">{lead.assignee.split(' ')[0]}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500 mt-auto">
        ...
        {/* Tags - Show max 2 with overflow */}
        {lead.tags && lead.tags.length > 0 && (
          <div className="flex items-center gap-1 min-w-0">
            {lead.tags.slice(0, 2).map((tag, index) => (
              <span
                key={index}
                className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full font-poppins truncate max-w-[40px]"
                title={tag}
              >
                {tag}
              </span>
            ...
            {lead.tags.length > 2 && (
              <span className="text-[10px] text-gray-500 flex-shrink-0" title={`+${lead.tags.length - 2} more tags`}>
                +{lead.tags.length - 2}
              </span>
            )}
          </div>
